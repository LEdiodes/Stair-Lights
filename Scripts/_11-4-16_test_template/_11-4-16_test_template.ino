...
// Variables
  ...
  strip.setBrightness(50);
    ...
    if (timeOut+5000 < millis() && timeOut+15000 < millis()) {    //switch off LED's in the direction of travel. was 10000
       if (downUp == 1) {
...
// Triggerred when motion is detected from the upstairs infrared sensor
 void topdown() {
    Serial.println ("detected top");
    colourWipeDown(strip.Color(255, 255, 255), 200 );                   // 4th number is delay
 }
 // Triggerred when motion is detected from the down stairs infrared sensor
 void bottomup() {
