...
// up broken\scatter
//down pretty good last 6 pixels down power on though
...
Adafruit_NeoPixel strip = Adafruit_NeoPixel(445, PIN, NEO_GRB + NEO_KHZ800); // the first variable is the number of LEDs
...
void loop() {
 ...
 void topdown() {
    ...
    colourWipeDown(strip.Color(0, 255, 0), 400 );     // Warm White - RGB - then 4th number is delay
 ...
 void bottomup() {
    Serial.println ("motion detected from the BOTTOM of the stairs");            // Helpful debug message
    colourWipeUp(strip.Color(255, 255, 255), 400);      // Warm White
      //for(int i=0; i<3; i++) {                   // Helpful debug indication flashes led on Arduino board twice
      //digitalWrite(ledPin,HIGH);
