...
Adafruit_NeoPixel strip = Adafruit_NeoPixel(446, PIN, NEO_GRB + NEO_KHZ800); // the first variable is the number of LEDs
...
//  }
 ...
 void topdown() {
    ...
    colourWipeDown(strip.Color(0, 0, 255), 255 );     // Warm White - RGB - then 4th number is delay
 ...
 void bottomup() {
    ...
    colourWipeUp(strip.Color(200, 0, 0), 50);      // Warm White
      //for(int i=0; i<3; i++) {                   // Helpful debug indication flashes led on Arduino board twice
      //digitalWrite(ledPin,HIGH);
      delay(1000);
      //digitalWrite(ledPin,LOW);
